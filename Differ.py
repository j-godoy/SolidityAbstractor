...
def load_dot_file(file_path, withoutTimeout=False):
    ...
    for line in dot_lines:
        if '->' in line:
            ...
            to_node = parts[1].split('[label=')[0].strip()
            edge_name = "constructor" if "[label=constructor]" in parts[1] else parts[1].split('[label="')[1].split('"')[0]
            if "?" in edge_name:
                continue
            total_edges += 1
...
def get_diff_prev(solidityabstractor_subjects, k_prev, k_curr, timeout):
    path_solidityabstractor_prev = f"C:\\Users\\JavierGodoy\\Repos\\SolidityAbstractor\\graph\\k_{k_prev}\\to_{timeout}"
    path_solidityabstractor_curr = f"C:\\Users\\JavierGodoy\\Repos\\SolidityAbstractor\\graph\\k_{k_curr}\\to_{timeout}"
...
# subjects_original = ["RefundEscrow_Mode.states"]
...
subjects_original = ["DigitalLocker_Mode.states"]
