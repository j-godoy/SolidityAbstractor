...
def get_valid_transitions_output(arg, preconditionsThread, preconditions, extraConditionsTemp, extraConditions, functions, statesThread, contractName, mode, statesNames, functionVariables, functionPreconditions, fileName, basic_mode, txBound, time_out, verbose, query_list, QUERY_TYPE, states, dot, tool_output, number_to, number_corral_fail, number_corral_fail_with_tackvars):
    temp_output = ""
    tempFunctionNames = []
    try:
            ...
            for indexPreconditionAssert, preconditionAssert in enumerate(preconditions):
                for indexFunction, function in enumerate(functions):
                    ...
                    if ((indexFunction + 1) in statesThread[indexPreconditionRequire] and mode == Mode.epa) or (mode == Mode.states):
                        ...
                        write_file(fileNameTemp, temp_output, contractName)
                        tool = "VeriSol " + fileNameTemp + " " + contractName
                        try_transaction(tool, tempFunctionNames, final_directory, statesThread, states, mode, functions, statesNames, basic_mode, txBound, time_out, verbose, query_list, QUERY_TYPE, dot, contractName, tool_output, number_to, number_corral_fail, number_corral_fail_with_tackvars, arg)
                        if not verbose:
...
def try_preconditions(tool, tempFunctionNames, final_directory, statesTemp, preconditionsTemp, extraConditionsTemp, basic_mode, txBound, time_out, mode, functions, statesNames, states, verbose, query_list, QUERY_TYPE, contractName, tool_output, number_to, number_corral_fail, number_corral_fail_with_tackvars, arg): 
    preconditionsTemp2 = []
    statesTemp2 = []
    ...
    for functionName in tempFunctionNames:
        ...
        if query_result[1] == TRACK_VARS:
            query_result = try_command(tool, functionName, tempFunctionNames, final_directory, statesTemp, txBound, time_out, True, mode, functions, statesNames, states, verbose, query_list, QUERY_TYPE, contractName, tool_output, number_to, number_corral_fail, number_corral_fail_with_tackvars)
        # query_result = try_command_parallel(functionName, tool, tempFunctionNames, final_directory, statesTemp, txBound, time_out, False, mode, functions, statesNames, states, verbose, query_list, QUERY_TYPE)
        success = query_result[0]
        if success:
            # print_combination(indexPreconditionRequire, statesTemp, mode)
...
def try_init(states, preconditionAssertTuple, mode, functions, statesNames, extraConditions, contractName, preconditions, functionVariables, functionPreconditions, fileName, basic_mode, txBound, time_out, verbose, query_list, QUERY_TYPE, dot, tool_output, number_to, number_corral_fail, number_corral_fail_with_tackvars):
    try:
        ...
        query_result = try_command(tool, functionName, tempFunctionNames, final_directory, [], txBound_constructor, time_out, False, mode, functions, statesNames, states, verbose, query_list, QUERY_TYPE, contractName, tool_output, number_to, number_corral_fail, number_corral_fail_with_tackvars)
        if query_result[1] == TRACK_VARS:
            query_result = try_command(tool, functionName, tempFunctionNames, final_directory, [], txBound_constructor, time_out, True, mode, functions, statesNames, states, verbose, query_list, QUERY_TYPE, contractName, tool_output, number_to, number_corral_fail, number_corral_fail_with_tackvars)
        success = query_result[0]
...
# que cada query se ejecute de forma paralela
# def try_command_parallel(functionName, tool, temp_function_name, tempFunctionNames, final_directory, statesTemp, txBound, time_out, trackAllVars, mode, functions, statesNames, states, verbose, query_list, QUERY_TYPE):
#     query_result = try_command(tool, functionName, tempFunctionNames, final_directory, statesTemp, txBound, time_out, False, mode, functions, statesNames, states, verbose, query_list, QUERY_TYPE)
#     if query_result[1] == TRACK_VARS: #Lo vuelvo a ejecutar, pero con el parámetro trackAllVars
#         query_result = try_command(tool, functionName, tempFunctionNames, final_directory, statesTemp, txBound, time_out, True, mode, functions, statesNames, states, verbose, query_list, QUERY_TYPE)
#     #TODO.....
#     return query_result# retorna una tupla (success, succes_by_timeout)


def try_command(tool, temp_function_name, tempFunctionNames, final_directory, statesTemp, txBound, time_out, trackAllVars, mode, functions, statesNames, states, verbose, query_list, QUERY_TYPE, contractName, tool_output, number_to, number_corral_fail, number_corral_fail_with_tackvars):
    # global tool_output, number_to, number_corral_fail, number_corral_fail_with_tackvars
    ...
    command = getToolCommand(temp_function_name, tool, tempFunctionNames, txBound, trackAllVars, contractName)
    if verbose:
       print(command)
    result = ""
    FAIL_TO = False
    ...
    if FAIL_TO:
        add_query_time(query_list, QUERY_TYPE, total_query_time, FAIL_TO, False)
    ...
    if output_error in output_verisol:
        if not trackAllVars:
            ...
            add_query_time(query_list, QUERY_TYPE, total_query_time, FAIL_TO, "fail_corral")
    ...
    add_query_time(query_list, QUERY_TYPE, total_query_time, FAIL_TO, feasible)
    return feasible, ""
...
def add_node_to_graph(indexPreconditionRequire, indexPreconditionAssert, indexFunction, statesTemp, states, succes_by_to, mode, functions, statesNames, dot):
    # dot.node(combinationToString(statesTemp[indexPreconditionRequire]), output_combination(indexPreconditionRequire, statesTemp, mode, functions, statesNames))
...
def reduceCombinations(arg, preconditionsThreads, statesThreads, extraConditionsThreads, fileName, functionVariables, contractName, basic_mode, txBound, time_out, mode, functions, statesNames, states, verbose, query_list, QUERY_TYPE, tool_output, number_to, number_corral_fail, number_corral_fail_with_tackvars):
    ...
    try:
        ...
        write_file(fileNameTemp, body, contractName)
        tool = "VeriSol " + fileNameTemp + " " + contractName
        preconditionsTemp2, statesTemp2, extraConditionsTemp2 = try_preconditions(tool, fuctionCombinations, final_directory, statesTemp, preconditionsTemp, extraConditionsTemp, basic_mode, txBound, time_out, mode, functions, statesNames, states, verbose, query_list, QUERY_TYPE, contractName, tool_output, number_to, number_corral_fail, number_corral_fail_with_tackvars, arg)
...
def validCombinations(arg, preconditionsThreads, statesThreads, extraConditionsThreads, mode, functions, statesNames, extraConditions, contractName, preconditions, functionVariables, functionPreconditions, fileName, basic_mode, txBound, time_out, verbose, query_list, QUERY_TYPE, states, dot, tool_output, number_to, number_corral_fail, number_corral_fail_with_tackvars):
    ...
    try:
        ...
        body, fuctionCombinations = get_valid_transitions_output(arg, preconditionsTemp, preconditions, extraConditionsTemp, extraConditions, functions, statesTemp, contractName, mode, statesNames, functionVariables, functionPreconditions, fileName, basic_mode, txBound, time_out, verbose, query_list, QUERY_TYPE, states, dot, tool_output, number_to, number_corral_fail, number_corral_fail_with_tackvars)
...
#         for indexPreconditionAssert, preconditionAssert in enumerate(preconditions):
#             executor.submit(try_init, states, (indexPreconditionAssert, preconditionAssert), fileName, contractName)
...
def main():
    ...
    def process_reduceCombinations():
        with Manager() as manager:
            # Crear las listas administradas dentro del bloque `with`
            preconditionsThreads2 = manager.list(preconditionsThreads)
            statesThreads2 = manager.list(statesThreads)
            extraConditionsThreads2 = manager.list(extraConditionsThreads)
            query_list2 = manager.list(query_list)

            # Utilizar ProcessPoolExecutor para ejecutar tareas en paralelo
    ...
    if mode == Mode.epa and not(reduced):
        ...
        # Llamada a la función y obtención de las listas fuera del bloque `with`
        preconditionsThreads, statesThreads, extraConditionsThreads, query_list = process_reduceCombinations()
        # for i in range(cant_preconditions):
        #     reduceCombinations (i, preconditionsThreads, statesThreads, 
        #                             extraConditionsThreads, fileName, functionVariables, contractName, 
        #                             basic_mode, txBound, time_out, mode, functions, statesNames, 
        #                             states, verbose, query_list, QUERY_TYPE)
    ...
    #         executor.submit(validCombinations, i, preconditionsThreads, statesThreads, extraConditionsThreads, mode, functions, statesNames, extraConditions, contractName, preconditions, functionVariables, functionPreconditions, fileName, basic_mode, txBound, time_out, verbose, query_list, QUERY_TYPE, states, dot, tool_output, number_to, number_corral_fail, number_corral_fail_with_tackvars)
    
    def process_validCombinations():
        with Manager() as manager:
            preconditionsThreads2 = manager.list(preconditionsThreads)
            statesThreads2 = manager.list(statesThreads)
            extraConditionsThreads2 = manager.list(extraConditionsThreads)
            query_list2 = manager.list(query_list)
            states2 = manager.list(states)
            dict_nodes_edges = manager.dict()
            ...
            return list(preconditionsThreads2), list(statesThreads2), list(extraConditionsThreads2), list(query_list2), list(states2)
        
    preconditionsThreads, statesThreads, extraConditionsThreads, query_list, states = process_validCombinations()
    ...
    def process_try_init():
        with Manager() as manager:
            ...
            return list(query_list2), list(states2)
    
    query_list, states = process_try_init()    
    # with concurrent.futures.ThreadPoolExecutor(max_workers=thread_workers) as executor:
    #     for indexPreconditionAssert, preconditionAssert in enumerate(preconditions):
    #         print(f"Submitting task init {indexPreconditionAssert}")  # Agregar un print para ver qué se está ejecutando
    #         executor.submit(try_init, states, (indexPreconditionAssert, preconditionAssert), mode, functions, statesNames, extraConditions, contractName, preconditions, functionVariables, functionPreconditions, fileName, basic_mode, txBound, time_out, verbose, query_list, QUERY_TYPE, dot, tool_output, number_to, number_corral_fail, number_corral_fail_with_tackvars)
    
    
    # with ProcessPoolExecutor(max_workers=thread_workers) as executor:
    #     for indexPreconditionAssert, preconditionAssert in enumerate(preconditions):
    #         executor.submit(try_init, states, (indexPreconditionAssert, preconditionAssert), contractName)
    print("ENDED")
...
def add_query_time(query_list, QUERY_TYPE, query_time, time_out, feasible):
